...
`tools/list` at `https://gateway.pipeworx.io/data-rennes/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_rennes_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"transport"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_rennes_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-rennes": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-rennes"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-rennes
```
